...
public class DriverSimulationService {
    ...
    public void initializeSimulation() {
        // Clear existing simulation data first
    ...
    // Clear existing simulation data to prevent duplicates
    private void clearExistingSimulationData() {
        try {
            // Find existing simulation drivers
            List<Driver> existingSimDrivers = driverRepository.findByLicensePlateStartingWith("SIM");

            // Remove from location tracking first
            for (Driver driver : existingSimDrivers) {
                try {
                    locationService.removeDriverLocation(driver.getId());
                } catch (Exception e) {
                    log.warn("Error removing location for driver {}: {}", driver.getId(), e.getMessage());
                }
            }

            // Delete from database
            if (!existingSimDrivers.isEmpty()) {
                driverRepository.deleteAll(existingSimDrivers);
                log.info("Cleared {} existing simulation drivers", existingSimDrivers.size());
            }

        } catch (Exception e) {
            log.warn("Error clearing existing simulation data: {}", e.getMessage());
    ...
    private void createSimulatedDrivers() {
        ...
        for (int i = 0; i < driverNames.length; i++) {
            try {
                String licensePlate = "SIM" + String.format("%03d", i + 1);

                // Double-check if driver exists (safety check)
                if (driverRepository.existsByLicensePlate(licensePlate)) {
                    log.warn("Driver with license plate {} already exists after cleanup, skipping", licensePlate);
                    continue;
                }
        ...
        log.info("Successfully created {} out of {} simulated drivers", successfulCreations, driverNames.length);
    }
    ...
    public void stopSimulation() {
        ...
        // Clear the simulated driver IDs list
        simulatedDriverIds.clear();
        log.info("Driver simulation stopped");
    }

    // Restart simulation (useful for testing)
    public void restartSimulation() {
        log.info("Restarting driver simulation...");
        stopSimulation();
        clearExistingSimulationData();
        createSimulatedDrivers();
        log.info("Driver simulation restarted with {} drivers", simulatedDriverIds.size());
    }
    ...
    public static class SimulationStats {
        ...
        public String toString() {
            return String.format("SimulationStats{total=%d, available=%d, busy=%d}",
